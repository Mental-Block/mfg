...
type EmailService interface {
	SendEmail(to string, subject string, mime string, msg string) (bool, error)
	VerificationEmailTemplate(token string)
}

func VerificationEmailTemplate(authToken string) string {
	return fmt.Sprintf(`
	<!DOCTYPE html>
<html>
<head>
          ...
          <tr>
            <td align="center" valign="top" style="padding: 2.5rem 1.5rem;">
              <a href="http://localhost:8080" target="_blank" style="display: inline-block;">
                <img src="./logo.png" width="auto" alt="Logo" border="0" style="display: block;">
              </a>
            </td>
          </tr>
        </table>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%%" style="max-width: 37.5rem;">
          ...
          <tr>
            <td align="left" bgcolor="#ffffff" style="padding: 1.5rem; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; font-size: 1rem; line-height: 1.5rem;">
              ...
              <p style="margin: 0;"><a href="http://localhost:8080" target="_blank">"http://localhost:8080/token"</a></p>
            </td>
          </tr>
          <tr>
...
func SendEmail(to auth.Email, subject string, mime string, msg string) (bool, error) {
	cfg := env.Env()

	auth := smtp.PlainAuth("", cfg.SMTP.HostEmail, cfg.SMTP.Password, cfg.SMTP.Host)

	newMsg := []byte(subject + mime + msg)

	err := smtp.SendMail(cfg.SMTP.Host+":"+cfg.SMTP.Port, auth, cfg.SMTP.HostEmail, []string{string(to)}, newMsg)
