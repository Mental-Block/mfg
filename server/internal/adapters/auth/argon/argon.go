...
func New(memory uint32, iterations uint32, parrallelism uint8, saltLength uint32, keyLength uint32) *Params {
	return &Params{
		Memory:      64 * 1024,
		Iterations:  1,
...
//
func (p *Params) CreateHash(password string) (hash string, err error) {
...
// false.
func (p *Params) ComparePasswordAndHash(password, hash string) (match bool, err error) {
	match, _, err = p.CheckHash(password, hash)
	return match, err
}
...
// should).
func (p *Params) CheckHash(password, hash string) (match bool, params *Params, err error) {
	params, salt, key, err := p.DecodeHash(hash)
	if err != nil {
...
func (p *Params) generateRandomBytes(n uint32) ([]byte, error) {
	b := make([]byte, n)
	_, err := rand.Read(b)
	if err != nil {
		return nil, err
...
func (p *Params) DecodeHash(hash string) (params *Params, salt, key []byte, err error) {
