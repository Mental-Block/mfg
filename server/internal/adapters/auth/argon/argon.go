...
func (p *Password) Hash(password domain.Password) domain.Password {
	return domain.Password(base64.StdEncoding.EncodeToString(argon2.Key([]byte(password), p.salt, 1, 32*1024, 4, 32)))
}

func (p *Password) Verify(password domain.Password, hash domain.Password) bool {
	newHash := p.Hash(password)
	return newHash == hash
}
